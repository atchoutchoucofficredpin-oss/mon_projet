...
function TransfertStockPage() {
  ...
  const [lieuxStockage, setLieuxStockage] = useState([]);
  ...
  const filteredStockDestinations = stocks.filter(
    stock => stock.id !== parseInt(selectedStockSource) &&
             (selectedProduit ? stock.produit.id === parseInt(selectedProduit) : false)
  );
    ...
    try {
      const response = await fetch('http://localhost:8000/api/transferts/', {
        ...
        body: JSON.stringify({
          ...
          stock_source_id: parseInt(selectedStockSource),
          stock_destination_id: parseInt(selectedStockDestination),
        ...
        <div className="liste-elements">
          {transferts.map(t => (
            <div key={t.id} className="carte-element">
              ...
              <p>De: {t.stock_source.lieu_stockage.nom}</p>
              <p>À: {t.stock_destination.lieu_stockage.nom}</p>
